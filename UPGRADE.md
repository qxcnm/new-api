...
本次功能支持保留现有数据和配置升级。新版主节点启动时自动为渠道表补充可空的 `model_groups` 文本字段；旧渠道默认继续继承渠道分组，现有令牌、模型基础价格和分组倍率无需重新配置。省略该字段的管理 API 更新也会保留已保存的绑定。

升级前按下文备份数据库，再替换新版程序或镜像并启动。单实例重启会短暂中断服务；本次验证覆盖数据库迁移和功能兼容，没有验证生产环境零停机切换。

多节点部署应先让新版主节点完成迁移，再升级其余节点；所有节点都运行新版后，才开始配置模型绑定。保存后当前节点立即刷新，其他节点按 `SYNC_FREQUENCY` 同步（默认 60 秒）；需要立即收紧访问范围时，应确认所有节点已刷新或重启后再接入请求。

启用绑定后，不要直接切回旧程序。旧版不能执行逐模型分组限制，可能放宽模型访问范围；需要回退时，先把限制转换成旧版支持的渠道配置并核对路由。

SQLite 3.50.4、MySQL 8.4.11、PostgreSQL 17.6 已完成新库及从 `v1.0.0-rc.37` 渠道结构升级的代表性数据测试，各执行两次迁移，第二次没有重复结构变更，原数据、索引和唯一约束保留。测试命令见[模型分组回归验证](docs/channel/other_setting.md#模型分组回归验证)。另在 PostgreSQL 15.19 上通过 14 项模拟上游真实 HTTP 用例，包含流式、缓存、固定价格、分组隔离、换绑和失败退款；累计扣费 35920 quota，用户、令牌、渠道和消费日志一致。
