...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if relayInfo.RelayMode == relayconstant.RelayModeMidjourneyImagine { //绘画任务，此类任务可重复
		...
		if originTask == nil {
			...
			if !service.ApplyChannelModelGroup(c, channel, service.CovertMjpActionToModelName(midjRequest.Action)) {
				return service.MidjourneyErrorWrapper(constant.MjRequestError, "origin_task_model_group_forbidden")
			}
